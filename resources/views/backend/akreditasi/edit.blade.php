            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb p-0">
                    <li class="breadcrumb-item">
                        <a href="{{ route('backend.akreditasi', $type) }}"
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card card-body">
                    <form method="POST" action="{{ route('backend.akreditasi.update', [$type, $data->id]) }}"
                        enctype="multipart/form-data">
                        {{ csrf_field() }}
                        <div class="form-group">
                            <label for="exampleInput1">Judul</label>
                            <input type="text" name="title" class="form-control" id="exampleInput1"
                                placeholder="Enter Judul" value="{{ $data->title }}">
                        ...
                        <div class="form-group">
                            ...
                            <select name="type_upload" class="form-control" id="type_upload">
                                ...
                                <option value="1" {{ $data->type_upload == 1 ? 'selected' : '' }}>Upload File</option>
                                <option value="2" {{ $data->type_upload == 2 ? 'selected' : '' }}>Upload Link</option>
                            </select>
                        ...
                        <div class="form-group" id="upload_file">
                            ...
                            <a href="{{ $data->files() }}" target="__blank">{{ $data->files() }}</a>
                            {{-- display error --}}
                            @if ($errors->has('file'))
                                <div class="text-danger">
                                    {{ $errors->first('file') }}
